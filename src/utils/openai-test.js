...
const OPENAI_KEY = import.meta.env.VITE_APP_OPENAI_KEY;
...
export async function fetchChartData(article) {
  ...
  let prompt3 = `create 2 different types graphs and provide short paragraph insight on statistical facts of this news article - "${article.content}". Return answer in json with keys "graph" and "analysis". "analysis" contains the short paragraph generated. "graph" is an array of objects in which each object has a title relating to the short paragraph insight, data : an array of numbers and tags : array of single word tags we're plotting the data against which contains corresponsing data for the types of graphs suggested, labels as an object containing keys "x_axis" and "y_axis" for the data and tags that will be plotted against each other. the data array and the tags array must always be of the same length and the length must not be less than 3`;
